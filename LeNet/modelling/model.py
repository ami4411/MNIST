...
class LeNet:
	...
	def saveResult(self, model):
		now = datetime.datetime.now()
		time = now.strftime("%Y%m%d%H%M%S")

		path = os.getcwd() #Main Folder
		dirResult = os.path.join(path+"/result", time)
		...
		return (print("Model is saved")) #procedural func
	...
	def train_model(self):
		lenetRelu = keras.models.Sequential([
		    ...
		    keras.layers.AveragePooling2D(), 
		    keras.layers.Flatten(), #The purpose of this layer is to transform its input to a 1-dimensional array that can be fed into the subsequent dense layers.
		...
		self.saveResult(lenetRelu)
		return (print("Finish modelling")) #procedural func
